...
def main():
    ...
    if set(adapter) != set(full_meta):
        raise RuntimeError(
            "Adapter/Test metadata universe mismatch"
        )

    print(
        "TEST_ADAPTER_ROWS = 40000",
        flush=True,
    )
    ...
    with surface_path.open(
        ...
        for number, h in enumerate(
            ...
            if rid not in adapter:
                raise RuntimeError(
                    f"H5000 row not in Adapter: {rid}"
                )
                ...
                json.dumps(
                    {
                        "schema_version": 1,
                        "experiment":
                            "final_fiveauthor_test_rich30_evaluation_v1",
    ...
    summary_path = (
        args.output_root
        / "final_fiveauthor_test_rich30_summary_v1.json"
    )

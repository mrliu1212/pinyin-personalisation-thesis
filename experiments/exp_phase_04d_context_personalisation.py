"""Run the frozen Phase 4D contextual-memory evaluation."""
...
from src.phase_04d_evaluation import evaluate_phase_04d
...
DEFAULT_OUTPUT = Path("results/experiments/phase_04d/evaluation.json")
...
def _print_condition(name: str, result: dict[str, Any]) -> None:
    ...
    if "rank_changes" in result:
        ...
        print(
            f"  improved={changes['improved']}, unchanged={changes['unchanged']}, "
            f"harmed={changes['harmed']}"
        )
    if "diagnostics" in result:
        diagnostics = result["diagnostics"]
        print(
            "  retrieval: "
            f"eligible={diagnostics['queries_with_eligible_same_pinyin_history']}, "
            f"nonzero={diagnostics['queries_with_nonzero_contextual_similarity']} "
            f"({_percent(diagnostics['percentage_with_nonzero_contextual_similarity'])}), "
            f"mean similarity={diagnostics['mean_retrieved_similarity']:.4f}, "
            f"mean max={diagnostics['mean_maximum_similarity']:.4f}"
        )


def run(zhu_path: Path, lu_path: Path, output_path: Path) -> dict[str, Any]:
    result = evaluate_phase_04d(read_jsonl(zhu_path), read_jsonl(lu_path))
    ...
    for subset_name, subset in result["subsets"].items():
        ...
        for condition, condition_result in subset.items():
            _print_condition(condition, condition_result)

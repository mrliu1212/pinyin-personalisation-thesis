# Initial+Short Standardized Personalisation Reproducibility

Date: 2026-08-21

This note records how to reproduce the standardized Initial+Short candidate-coverage, candidate-scoring, recovery, and controllability findings without using Dev3000 or Test. Sections 1–16 preserve the earlier v3 record; Section 17 adds the current recovery and Context–Preference–Confidence phase.
...
# 17. POST-v3 UPDATE — Recovery, Selector, Concentration, and Context–Preference–Confidence

This section extends the earlier candidate-coverage / candidate-scoring reproducibility record through the current recovery and controllability experiments.

## 17.1 Scope update

The main Initial-Pinyin line now includes:

```text
candidate availability
-> candidate scoring
-> NGramSelector K ablation
-> NGram-CS recovery interpolation
-> frequency/concentration calibration
-> Context–Preference–Confidence two-anchor sweep
```

The current main line remains K5. K10 is retained only as historical exploratory evidence.

Protocol assertions remain unchanged:

```text
Gold used for candidate construction = false
Gold used for scoring = false
Gold used for Train-Val evaluation/model selection = true
Dev3000 used = false
Test used = false
```

## 17.2 NGramSelector K1/K3/K5
...
| K | Macro | Micro | Top3 | Top5 | MRR | Missing | Rec@3 | Rec@10 | PV1 net |
|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
...
K3 remains the selector-family development point because the pre-specified primary metric is Macro-author Top1.

## 17.3 NGram-CS interpolation
...
```text
6P+2CS: Macro=.400212 Top3=.615731 Top5=.685669 MRR=.534314 Missing=.244392
4P+4CS: Macro=.402628 Top3=.612593 Top5=.683461 MRR=.535609 Missing=.245613
```

## 17.4 Frequency-only K5 + query concentration
...
Entropy only marginally changes overall accuracy, although recovery increases. This is retained as evidence that query-level confidence is not sufficient without stronger candidate-specific scoring.

## 17.5 Latest two-anchor runner
...
Frozen Interpolated NGram:

```text
K5|Interpolated|maxN=2|kappa=1|tau=2048
```
...
Exact lambda_E=0 anchor reproduction:

```text
Top3Anchor PASSED
BalancedAnchor PASSED
```

## 17.6 Full current two-anchor grid
...
## 17.7 Current development operating points

```text
Overall balanced:
B + 4P + 4CS + 2E
Macro=.404807, Micro=.429364, MRR=.537433
...
B + 6P + 2CS + 4E
Rec@3=.6407 but Macro=.371792
```

`NGramSelector@K3` remains a main alternative architecture because its Macro=.403964 is close to the balanced joint model while Rec@3=.6051 is strong and its WHO-vs-HOW-STRONGLY decomposition is particularly transparent.

## 17.8 Controllability interpretation

Current evidence supports two distinct control dimensions:

```text
Ranking Preference
  - first-choice / balanced
  - Top3-oriented
  - Top5-oriented

Personalisation Strength / Recovery Breadth
  - lambda_E in the joint scoring family
  - K in the Selector family
```

Do not claim that increasing the P_NG coefficient monotonically increases recoverability. The strongest empirical recovery-strength control is lambda_E, while beta:gamma primarily changes Context-vs-Preference weighting.

## 17.9 Latest output files
...
Use `artifact_checksums.json` or local `Get-FileHash` output to freeze exact artifact hashes. Do not guess hashes from filenames.

## 17.10 Latest documentation hashes

```text
11_INITIAL_PERSONALISATION_EVALUATION_METRICS_AND_DESIGN.md
534b5d5a091671a3d1e955fde951de613da4d0a1aded8e0a82d146be5299761f

12_INITIAL_PERSONALISATION_METRIC_PURPOSE_AND_PRIORITIES.md
9827d461c0ed81fe1ede3b9f35592986fc8be24f8b7d3a39fd2cd8da5fc0d38e

13_INITIAL_PERSONALISATION_CURRENT_CONCLUSIONS_AND_CONTROLLABILITY.md
3a73881188de740a6e8022d07931fdce8013d9f81f0b4529ffa1c2021230de78
```

## 17.11 Updated freeze checklist

```powershell
$root = '.\results\personalisation\initial_recovery_comparison_v1'

Get-FileHash '.\experiments\initial_personalisation\run_initial_ngram_cs_entropy_two_anchors_v1.py' -Algorithm SHA256
Get-FileHash "$root\pv1_ngram_selector_k135_v1\comparison.json" -Algorithm SHA256
Get-FileHash "$root\recovery_ngram_cs_interpolation_k5_v1\comparison.json" -Algorithm SHA256
Get-FileHash "$root\pv1_ngram_k5_additive_concentration_v1\comparison.json" -Algorithm SHA256
Get-FileHash "$root\pv1_ngram_k5_joint_frequency_concentration_v1\comparison.json" -Algorithm SHA256
...
```text
Train-Val development result only
Dev3000 untouched
Test untouched
no significance claim without paired testing
```

"""Focused lambda-extension follow-up for Initial-Pinyin K5 NGram+CS recovery.

Purpose
-------
The previous unified K5 experiment selected NGram+CS at the upper lambda grid
edge (lambda=16) while still trailing frozen PV1 slightly on Macro-author Top1.
This follow-up changes ONE thing only: extend the NGram+CS recovery-strength
lambda grid beyond 16.
...
- Same already-selected K5 Interpolated-NGram supports; NGram is not recomputed
  or retuned here.
- No concentration term is used in this follow-up.
...
For each frozen Personal-K5 candidate c:
    ...
    score_personal(c) = generic_boundary + lambda_D * evidence(c)
...
Default lambda extension
------------------------
{16, 24, 32, 48, 64}

Outputs are written to a new output root and do not overwrite the previous
NGram+CS+Concentration experiment.
"""
...
# Follow-up justified by the previous selected NGram+CS lambda landing at the old
# upper grid edge (16). This script tests only the pre-declared extension values.
# If 64 is selected, the output flags the new grid edge.
DEFAULT_LAMBDAS = (16.0, 24.0, 32.0, 48.0, 64.0)
DEFAULT_GAMMAS = (0.0,)

FAMILY_ORDER = ("NGram+CS",)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Focused K5 NGram+CS lambda extension vs frozen PV1"
    )
    ...
    parser.add_argument(
        ...
        help="must remain 0 in this focused no-concentration follow-up",
    )
    ...
    if any(value <= 0.0 for value in lambdas):
        raise RuntimeError("Lambda extension values must be positive")
    if gammas != (0.0,):
        raise RuntimeError("This focused follow-up fixes gamma=0 and does not test concentration")

    args.output_root.mkdir(parents=True, exist_ok=True)
    provenance = verify_frozen_inputs(args)
    selected_ngram_key = str(provenance["selected_k5_interpolated_key"])

    print("=== INITIAL K5 NGRAM+CS LAMBDA EXTENSION ===")
    print(f"Primary comparator: PV1")
    print(f"Frozen K5 Interpolated method: {selected_ngram_key}")
    print(f"lambda_D grid: {lambdas}")
    print("concentration: OFF (gamma fixed at 0)")
    print("families:")
    ...
    feature_summary = {
        "schema_version": 1,
        "status": "complete",
        "experiment": "initial_k5_ngram_cs_lambda_extension_v1",
    ...
    grid_payload = {
        ...
        "selection_metric": "Macro-author Top1; tie MRR@10, then lower lambda",
        "selected_k5_interpolated_key": selected_ngram_key,
    ...
    comparison = {
        ...
        "experiment": "initial_k5_ngram_cs_lambda_extension_v1",
        "research_question": (
            "Was lambda=16 a truncated optimum for frozen K5 Interpolated-NGram + Choice Share "
            "recovery, and can a stronger recovery weight improve over frozen PV1?"
        ),
        "primary_comparator": "PV1",
        "selected_k5_interpolated_key": selected_ngram_key,
        "population": {
        ...
        "method_definitions": {
            ...
            "concentration": "disabled; gamma fixed at 0",
        },
        "invariants": {
    ...
    print("\n=== LAMBDA EXTENSION RESULTS (PRIMARY COMPARATOR = PV1) ===")
